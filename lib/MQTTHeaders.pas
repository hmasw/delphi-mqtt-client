...
type
  ...
  public
    class function EncodeStringToBytes(AStrToEncode: string): TBytes;
    class function EncodeStringToBytesNoLength(AStrToEncode: string): TBytes;
...
class function TMQTTUtilities.EncodeStringToBytes(AStrToEncode: string): TBytes;
var
  i: integer;
  temp: TBytes;
begin
...
class function TMQTTUtilities.EncodeStringToBytesNoLength(AStrToEncode: string): TBytes;
var
  temp: TBytes;
  i: integer;
begin
...
begin
  ...
  try
    ClearField;
    AddField(TMQTTUtilities.EncodeStringToBytes(Self.PROTOCOL_ID));
...
begin
  ...
  begin
    ...
    begin
      lineAsBytes := TMQTTUtilities.EncodeStringToBytesNoLength(line);
      AppendToByteArray(lineAsBytes, Result);
    end
    else
    begin
      if (aWithIntegerLiterals and TryStrToInt(line, lineAsInt))then
      begin
        AppendToByteArray(Lo(lineAsInt), Result);
      end
      else
      begin
        lineAsBytes := TMQTTUtilities.EncodeStringToBytes(line);
        AppendToByteArray(lineAsBytes, Result);
      end;
    end;
  end;
...
begin
  ClearField;
  AddField(TMQTTUtilities.EncodeStringToBytes(FTopic));
  if (FQoSLevel > 0) then
  begin
